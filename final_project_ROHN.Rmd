...
plot.test(numvar, vars, gavg) <- function(){
if(numvar == 1){
  #plot survival months by recoded marital status
ggplot(seer, aes(x=as.factor(vars), y=surv.months)) +
...
#kaplan-meier
km_fit <- survfit(Surv(surv.months) ~ part, data=seer)
ggsurvplot(km_fit, data = seer, pval = T, conf.int = T)
#cox proportional hazards regression
cox <- coxph(Surv(surv.months) ~ part, data = seer)
summary(cox)
}
}
...
#kaplan-meier
km_fit <- survfit(Surv(surv.months) ~ sex, data=seer)
ggsurvplot(km_fit, data = seer, pval = T)
#cox proportional hazards regression
cox <- coxph(Surv(surv.months) ~ sex, data = seer)
summary(cox)
...
#kaplan-meier
km_fit <- survfit(Surv(surv.months) ~ part + sex, data=seer)
ggsurvplot(km_fit, data = seer, pval = T, conf.int = T)
#cox proportional hazards regression
...
#kaplan-meier
km_fit <- survfit(Surv(surv.months) ~ part + sex, data=seer1)
gg <- ggsurvplot(km_fit, data = seer1, pval = T, conf.int = T)
gg$plot + facet_wrap(~ sex)
#cox proportional hazards regression
cox <- coxph(Surv(surv.months) ~ part + sex + race, data = seer1)
summary(cox)
